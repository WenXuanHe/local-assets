var CHAT_USER = {
    ...
    showMessage: function(msg, from, type) 
    {
        ...
        if (name)
        {
            ...
            {
                if(window.showInputingTimer) 
                {
                    clearInterval(showInputingTimer);
                }

                $(".chat-typing").html("客服正在输入...");
                window.showInputingTimer = setInterval(function()
                {
                    $(".chat-typing").html("");
                }, 3000);
            ...
            if (CHAT_USER.timers.waitingTimer) 
            {
                window.clearTimeout(CHAT_USER.timers.waitingTimer);
        ...
        $(".chat-content section").last().get(0).scrollIntoView();
    ...
    overDialogue: function() {
        if (confirm("确定要结束对话吗？结束对话后窗口将在3秒后关闭")) 
        {
            ...
            setTimeout(function()
            {
                $(CHAT_USER).triggerHandler('overDialogue');
            }, 3000);
        }
    },
    ping: function() {
        CHAT.connection.send($iq({
            to: CHAT.SERVER_NAME,
            type: "get",
            id: "ping"
        }).c("ping", {
