(function()
{
    ...
    var uiName = 'custominsertimage',
        template = '<div class="image-upload"> <div class="kf5-upload-drop-area" style="position: relative; overflow: hidden; direction: ltr;"><span>拖动图片到此或点击上传</span> <input type="file" name="file" style="position: absolute; right: 0px; top: 0px; font-family: Arial; width: 400px; height: 60px; margin: 0px; padding: 0px; cursor: pointer; opacity: 0;"> </div> </div> <div class="image-view"></div>';
    ...
    function customInsertImage(editor)
    {
        ...
        var historyXHR = $.get('/posts/ImageHistoryJSON', function(data)
        {
            ...
            function loadImage(image)
            {
                var img = document.createElement('img'),
                    $imageItem = $('<div class="image-item"><div class="attach-loading">加载中...</div></div>');

                img.src = '/attachments/download/' + image.id + '/' + image.token;
                $(img).attr('token', image.token);

                dialog.$element.find('.image-view').append($imageItem);
                img.onload = function()
                {
                    img.onload = null;
            ...
            dialog.$element.find('.image-view').on('scroll', function(e)
            {
        ...
        {
            var $lastItem = $('<div class="image-item"><div class="attach-loading">上传中...</div></div>')
                    .attr('queueId', id);

            ++uploading;
            
            dialog.$element.find('.image-view').prepend($lastItem);
        }

        function imageUploadSuccess(id, fileName, data)
        {
            var src = '/attachments/download/' + data.id + '/' + data.token,
